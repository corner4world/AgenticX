...
import { boolean, index, int, json, mysqlTable, datetime, uniqueIndex, varchar } from "drizzle-orm/mysql-core";
...
export const policyRulePacks = mysqlTable(
  "policy_rule_packs",
  {
    ...
    appliesTo: json("applies_to").$type<PolicyAppliesTo>().notNull().default({}),
    ...auditColumns,
  },
  (table) => ({
...
export const policyRules = mysqlTable(
  "policy_rules",
  {
    ...
    appliesTo: json("applies_to").$type<PolicyAppliesTo>(),
...
export const policyRuleVersions = mysqlTable(
...
export const policyPublishEvents = mysqlTable(
