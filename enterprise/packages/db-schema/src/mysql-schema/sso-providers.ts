...
export const ssoProviders = mysqlTable(
  "sso_providers",
  {
    ...
    scopes: json("scopes").$type<SsoProviderRowScopes>().notNull().default(["openid", "profile", "email"]),
    claimMapping: json("claim_mapping").$type<SsoProviderClaimMapping>().notNull().default({}),
    samlConfig: json("saml_config").$type<SsoProviderSamlConfig>(),
    defaultRoleCodes: json("default_role_codes").$type<SsoProviderDefaultRoleCodes>().notNull().default(["member"]),
