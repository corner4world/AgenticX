...
#   bash scripts/bootstrap.sh --db=postgresql   # 业务库方言（默认 postgresql）
#   bash scripts/bootstrap.sh --db=mysql        # 使用 MySQL 8.0 开发栈
#   bash scripts/bootstrap.sh --reset-db        # 清库后重建（危险，仅限开发）
#   bash scripts/bootstrap.sh --skip-docker     # 即使 local 模式也不起 docker（本地已有库）
#
# 运行后仍需启动服务：
#   bash scripts/start-dev.sh
#   # 或带中间件：bash scripts/start-dev-with-infra.sh --db=postgresql|mysql
...
for arg in "$@"; do
  case "$arg" in
    ...
    --db=postgresql|--db=postgres) DB_DIALECT="postgresql"; DB_DIALECT_EXPLICIT=1 ;;
    --db=mysql) DB_DIALECT="mysql"; DB_DIALECT_EXPLICIT=1 ;;
    --reset-db)    RESET_DB=1 ;;
...
if [[ "$DB_DIALECT" != "postgresql" && "$DB_DIALECT" != "mysql" ]]; then
  echo "[bootstrap] 非法 --db=$DB_DIALECT（仅 postgresql|mysql）" >&2
  exit 1
fi
...
# --- 3.1 DATABASE_DIALECT + DATABASE_URL ---
# Priority: explicit --db > existing DATABASE_DIALECT / URL scheme > default postgresql.
if [ "$DB_DIALECT_EXPLICIT" -eq 0 ]; then
  if [ -n "${DATABASE_DIALECT:-}" ]; then
    case "${DATABASE_DIALECT}" in
      mysql|postgresql) DB_DIALECT="$DATABASE_DIALECT" ;;
    esac
  elif [ -n "${DATABASE_URL:-}" ]; then
    case "$DATABASE_URL" in
      mysql://*) DB_DIALECT=mysql ;;
      postgres://*|postgresql://*) DB_DIALECT=postgresql ;;
    esac
  fi
fi
...
if [ -z "${DATABASE_URL:-}" ]; then
  if [ "$MODE" = "local" ]; then
    ...
    ok "default DATABASE_DIALECT=$DATABASE_DIALECT DATABASE_URL=$DATABASE_URL"
  else
    err "server 模式必须外部提供 DATABASE_URL（与 DATABASE_DIALECT=$DATABASE_DIALECT 匹配）"
    exit 1
  fi
else
  case "$DATABASE_URL" in
    mysql://*)
      if [ "$DATABASE_DIALECT" != "mysql" ]; then
        err "DATABASE_URL 为 mysql:// 但 DATABASE_DIALECT=$DATABASE_DIALECT；请对齐或传 --db=mysql"
        exit 1
      fi
      ;;
    postgres://*|postgresql://*)
      if [ "$DATABASE_DIALECT" != "postgresql" ]; then
        err "DATABASE_URL 为 postgres(ql):// 但 DATABASE_DIALECT=$DATABASE_DIALECT；请对齐或传 --db=postgresql"
        exit 1
      fi
      ;;
    *)
      warn "DATABASE_URL scheme 未识别为 postgres/mysql；继续使用 DATABASE_DIALECT=$DATABASE_DIALECT"
      ;;
  esac
  ok "DATABASE_DIALECT=$DATABASE_DIALECT DATABASE_URL=(set)"
fi
...
if [ "$MODE" = "local" ] && [ "$SKIP_DOCKER" -eq 0 ]; then
  if [ "$RESET_DB" -eq 1 ]; then
    warn "--reset-db: 即将销毁本地数据库数据卷（postgresql + mysql profiles）"
    (cd "$(dirname "$COMPOSE_FILE")" && docker compose -f "$(basename "$COMPOSE_FILE")" --profile postgresql --profile mysql down -v)
  fi
  log "starting $DATABASE_DIALECT + redis via docker compose (profile=$DATABASE_DIALECT)"
  (cd "$(dirname "$COMPOSE_FILE")" && docker compose -f "$(basename "$COMPOSE_FILE")" --profile "$DATABASE_DIALECT" up -d redis)
  if [ "$DATABASE_DIALECT" = "mysql" ]; then
    (cd "$(dirname "$COMPOSE_FILE")" && docker compose -f "$(basename "$COMPOSE_FILE")" --profile mysql up -d mysql)
    log "waiting for mysql healthy..."
    for i in $(seq 1 90); do
      if docker exec agenticx-mysql-dev mysqladmin ping -h 127.0.0.1 -uroot -proot --silent >/dev/null 2>&1; then
        ok "mysql is ready"
        break
      fi
      sleep 1
      if [ "$i" -eq 90 ]; then err "mysql 启动超时"; exit 1; fi
    done
  else
    (cd "$(dirname "$COMPOSE_FILE")" && docker compose -f "$(basename "$COMPOSE_FILE")" --profile postgresql up -d postgres)
...
#################################
export DATABASE_DIALECT
export DATABASE_URL
BOOTSTRAP_LOG="$(agx_new_log_file bootstrap)"
log "running db:migrate dialect=$DATABASE_DIALECT (log: $BOOTSTRAP_LOG)"
(
  cd "$ENTERPRISE_DIR"
...
log "running db:seed dialect=$DATABASE_DIALECT (log: $BOOTSTRAP_LOG)"
(
  cd "$ENTERPRISE_DIR"
